  ...
  return (
    <div
      ...
      style={{
        ...
        fontFamily: profile.appearance.font,
      }}
    >
      <div className="w-full max-w-2xl mx-auto">
        ...
        <div className="text-center my-8">
          ...
          <h1 className="font-bold text-2xl" style={{ color: profile.appearance.buttonColor }}>
            {profile.displayName}
          </h1>
          <p className="mt-2" style={{ color: profile.appearance.buttonColor }}>
            {profile.bio}
          </p>
        </div>
        ...
        <div className="space-y-4">
          {links.map((link) => (
            <a
              key={link._id}
              href={link.url}
              // THIS IS THE NEW LINE: It calls our tracking function on click
              onClick={() => handleLinkClick(link._id)}
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full text-center p-4 rounded-lg font-semibold transition-transform hover:scale-105"
              style={{
                backgroundColor: profile.appearance.buttonColor,
                color: profile.appearance.backgroundColor,
              }}
            >
              {link.title}
            </a>
